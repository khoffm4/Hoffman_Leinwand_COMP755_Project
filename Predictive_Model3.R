...
#Log transofmrm (abs + log)
Train_sign = Train_resids >= 0 
Test_sign = Test_resids >= 0 

Transform_Train = sqrt(abs(Train_resids))
Transform_Test = sqrt(abs(Test_resids))

Train_resids = Transform_Train
Test_resids = Transform_Test
...
fitted = fitted^2
fitted[!c(Train_sign, Test_sign)] = fitted[!c(Train_sign, Test_sign)] * -1
